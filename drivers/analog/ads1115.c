                                      ...
                                      FAR struct adc_msg_s *msg)
{
  ...
  channel_bits = CHANNEL_BITS(msg->am_channel);
  ...
  if (new_channel || ONESHOT_MODE(priv))
    {
                         ...
                         unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case ANIOC_ADS1115_SET_COMP_QUEUE:
      {
        DEBUGASSERT(arg >= ADS1115_COMP_QUEUE1 && arg <= ADS1115_COMP_QUEUE4);
