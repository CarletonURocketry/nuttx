/****************************************************************************
 * boards/arm/rp2040/cuinspace-pico-w/src/rp2040_bringup.c
 *
...
#define CYW43439_CLOCK_GPIO 29
  ...
  g_cyw43439 = rp2040_cyw_setup(CYW43439_POWER_ON_GPIO,
                                CYW43439_CHIP_SELECT_GPIO, CYW43439_DATA_GPIO,
                                CYW43439_CLOCK_GPIO, CYW43439_DATA_GPIO);

  if (g_cyw43439 == NULL) {
    ret = errno;

    syslog(LOG_ERR, "Failed to initialize cyw43439 (WiFi chip): %d\n", ret);
